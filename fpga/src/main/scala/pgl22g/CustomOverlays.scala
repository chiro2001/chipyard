...
import sifive.fpgashells.shell.pango.pgl22gshell.{PGL22GDDRSize, PGL22GShellBasicOverlays}

class SysClock2PGL22GPlacedOverlay(val shell: PGL22GShellBasicOverlays, name: String, val designInput: ClockInputDesignInput, val shellInput: ClockInputShellInput)
...
class SysClock2PGL22GShellPlacer(shell: PGL22GShellBasicOverlays, val shellInput: ClockInputShellInput)(implicit val valName: ValName)
  extends ClockInputShellPlacer[PGL22GShellBasicOverlays]
{
    def place(designInput: ClockInputDesignInput) = new SysClock2PGL22GPlacedOverlay(shell, valName.name, designInput, shellInput)
}

case object PGL22GDDR2Size extends Field[BigInt](0x40000000L * 2) // 2GB
class DDR2PGL22GPlacedOverlay(val shell: PGL22GFPGATestHarness, name: String, val designInput: DDRDesignInput, val shellInput: DDRShellInput)
  extends DDRPlacedOverlay[PangoPGL22GMIGPads](name, designInput, shellInput)
{
  ...
  shell { InModuleBody {
    require (shell.sys_clock2.get.isDefined, "Use of DDRPGL22GOverlay depends on SysClock2PGL22GOverlay")
...
class DDR2PGL22GShellPlacer(shell: PGL22GFPGATestHarness, val shellInput: DDRShellInput)(implicit val valName: ValName)
  extends DDRShellPlacer[PGL22GFPGATestHarness] {
  def place(designInput: DDRDesignInput) = new DDR2PGL22GPlacedOverlay(shell, valName.name, designInput, shellInput)
}
