...
case class AnalogSourceAnnotation(target: ComponentName, pin: String) extends SingleTargetAnnotation[ComponentName] {
  def duplicate(n: ComponentName) = this.copy(target = n)
}

/** A module, e.g. ExtModule etc., that should add the input pin */
case class AnalogSinkAnnotation(target: Named, pin: String) extends SingleTargetAnnotation[Named] {
 ...
 */
class AnalogWiringTransform extends Transform with DependencyAPIMigration {
  ...
  def execute(state: CircuitState): CircuitState = {
    val annos = state.annotations.collect {
      case a @ (_: AnalogSinkAnnotation | _: AnalogSourceAnnotation) => a
    }
    annos match {
      case Seq() => state
      case p =>
        ...
        val errors = p.flatMap {
          case AnalogSinkAnnotation(m, pin) =>
            sinks(pin) = sinks.getOrElse(pin, Seq.empty) :+ m
            None
          case AnalogSourceAnnotation(c, pin) =>
        ...
        println(s"source and sinks: ${sources} ${sinks}")
