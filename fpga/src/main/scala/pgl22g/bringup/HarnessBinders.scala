...
class WithBringupUART extends ComposeHarnessBinder({
  (system: HasPeripheryUARTModuleImp, th: BaseModule with HasHarnessSignalReferences, ports: Seq[UARTPortIO]) => {
    th match { case pgl22g: BringupPGL22GFPGATestHarnessImp => {
      require(ports.size == 2)

      pgl22g.bringupOuter.io_fmc_uart_bb.bundle <> ports.last
    } }
  }
})
//
...
//
//       pgl22g.bringupOuter.io_i2c_bb.bundle <> ports.head
//     } }
//   }
// })
//
...
//     th match { case pgl22g: BringupPGL22GFPGATestHarnessImp => {
//       (pgl22g.bringupOuter.io_gpio_bb zip ports).map { case (bb_io, dut_io) =>
//         bb_io.bundle <> dut_io
//       }
//     } }
//   }
  ...
  (system: HasPeripheryTSIHostWidget, th: BaseModule with HasHarnessSignalReferences, ports: Seq[Data]) => {
    th match { case pgl22g: BringupPGL22GFPGATestHarnessImp => {
      ...
      ports.head match { case tlPort: HeterogeneousBag[TLBundle] =>
        val tsiBundles = pgl22g.bringupOuter.tsiDdrClient.out.map(_._1)
      ...
      ports.last match { case serialPort: TSIHostWidgetIO =>
        pgl22g.bringupOuter.io_tsi_serial_bb.bundle <> serialPort
      }
    } }
  }
})
